...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty) (means map (mean => (mean, GenSeq.empty))).toMap else points groupBy (findClosest(_, means))  
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    else (1 until oldMeans.length).map(i => oldMeans(i).squareDistance(newMeans(i)) <= eta).reduce(_ && _) 
  }
